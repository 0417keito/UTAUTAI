...
def get_scheduler(cfg, optimizer):
    if cfg.scheduler_name.lower() == "eden":
        scheduler = Eden(optimizer, 5000, 4, warmup_batches=cfg.warmup_steps)
    elif cfg.scheduler_name.lower() == "noam":
        scheduler = NoamScheduler(cfg.base_lr, optimizer, cfg.decoder_dim,
                                  warmup_steps=cfg.warmup_steps)
    elif cfg.scheduler_name.lower() == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            cfg.warmup_steps, optimizer, eta_min=cfg.base_lr
        )
    else:
        raise NotImplementedError(f"{cfg.scheduler_name}")
...
def get_optimizer(cfg, model):
    if cfg.train_stage:
        model_parameters = model.stage_parameters(cfg.train_stage)
    else:
        model_parameters = model.parameters()
    
    if cfg.optimizer_name == "ScaledAdam":
        parameters_names = []
        if cfg.train_stage:
            parameters_names.append([name_param_pair[0] for name_param_pair in model.stage_named_parameters(cfg.train_stage)])
        else:
            parameters_names.append([name_param_pair[0] for name_param_pair in model.named_parameters()])
        
        optimizer = ScaledAdam(
            model_parameters, lr=cfg.base_lr, betas=(0.9, 0.95),
            clipping_scale=2.0, parameters_names=parameters_names,
            show_dominant_parameters=False, clipping_update_period=1000
        )
    elif cfg.optimizer_name == "Eve":
        optimizer = Eve(
            model_parameters, lr=cfg.base_lr,
            betas=(0.9, 0.98), target_rms=0.1
        )
    elif cfg.optimizer_name == "AdamW":
        optimizer = torch.optim.AdamW(
            model_parameters, lr=cfg.base_lr,
            betas=(0.9, 0.95), weight_decay=1e-2, eps=1e-8
        )
    elif cfg.optimizer_name == "Adam":
        optimizer = torch.optim.Adam(
            model_parameters, lr=cfg.base_lr,
            bets=(0.9, 0.95), eps=1e-8
        )
    else:
        raise NotImplementedError()
